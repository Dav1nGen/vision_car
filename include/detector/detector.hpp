...
void Detector::SignalLightDetect() {
  ...
  std::vector<std::vector<cv::Point>> contours;
  cv::findContours(this->processed_img, contours, cv::RETR_EXTERNAL,
                   cv::CHAIN_APPROX_SIMPLE);
  ...
  cv::circle(processed_img, centroids[0], 1, cv::Scalar(255, 255, 255), 1, 8,
             0);
  ...
  cv::Point2f rect_points[4];  // point[0]左下:x最小 point[1]左上:y最小
                               // point[2]右上:x最大 point[3]右下:y最大
  ...
  // 绘制旋转矩形及其顶点
  cv::polylines(processed_img, boxPointsCov, true, cv::Scalar(255, 255, 255),
                2);
  cv::circle(processed_img, center, 1, cv::Scalar(255, 255, 255), 1, 8, 0);
  cv::Point2f rect_point((rect_points[0].x + rect_points[2].x) / 2,
                         (rect_points[0].y + rect_points[2].y) / 2);
  ...
  for (auto contour_1 = contours.begin(); contour_1 != contours.end();
    ...
    for (auto contour_2 = contour_1 + 1; contour_2 != contours.end();
      ...
      if (rect_1.center.y - rect_2.center.y < 100) {
        cv::Point2f rect_points_1[4];  // point[0]左下:x最小 point[1]左上:y最小
                                       // point[2]右上:x最大 point[3]右下:y最大
        ...
        cv::polylines(black_img, boxPointsCov_1, true,
                      cv::Scalar(255, 255, 255), 2);

        cv::Point2f rect_points_2[4];  // point[0]左下:x最小 point[1]左上:y最小
                                       // point[2]右上:x最大 point[3]右下:y最大
        ...
        cv::polylines(black_img, boxPointsCov_2, true,
                      cv::Scalar(255, 255, 255), 2);

        cv::Point2f rect_1_center_point =
            cv::Point2f((rect_points_1[0].x + rect_points_1[2].x) / 2,
                        (rect_points_1[0].y + rect_points_1[2].y) / 2);
        cv::Point2f rect_2_center_point =
            cv::Point2f((rect_points_2[0].x + rect_points_2[2].x) / 2,
                        (rect_points_2[0].y + rect_points_2[2].y) / 2);
        this->targrt_point =
            cv::Point2f((rect_1_center_point.x + rect_2_center_point.x) / 2,
                        (rect_1_center_point.y + rect_2_center_point.y) / 2);
        cv::circle(black_img, this->targrt_point, 3, cv::Scalar(255, 255, 255),
                   1, 8, 0);
        cv::circle(black_img, this->car_direction, 1, cv::Scalar(255, 255, 255),
                   1, 8, 0);
        debug_img = black_img.clone();
        return true;
      }
...
void Detector::DirectControl() {
  ...
  if (this->targrt_point.x == 0 && this->targrt_point.y == 0) {
    ...
    // 向右修正
    if (this->car_direction.x < this->targrt_point.x &&
        abs(this->car_direction.x - this->targrt_point.x) < 50) {
